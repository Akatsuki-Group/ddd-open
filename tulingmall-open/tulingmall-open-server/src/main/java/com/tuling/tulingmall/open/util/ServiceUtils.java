...
public class ServiceUtils {
	...
	private static ServiceResponseEntity doSend(String sysId, String data) {
		...
		try {
			// 获取系统信息
			ClientInfo clientInfo = ConfigUtils.getClientInfo(sysId);
			// 无效的系统标识
			if (clientInfo == null) {
			...
			// 消息推送参数
			String notifyParam = clientInfo.getNotifyParam();
			// 执行推送
			if (StringUtils.isBlank(notifyParam)) {
				responseData = CommonUtils.sendHttpBodyRequest(clientInfo.getNotifyUrl(), data);
			} else {
				responseData = CommonUtils.sendHttpParameterRequest(clientInfo.getNotifyUrl(), notifyParam, data);
			}
			message = "发送成功";
