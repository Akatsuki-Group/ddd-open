...
import com.tuling.tulingmall.open.entity.GsResponse;
import com.tuling.tulingmall.open.entity.RequestMsgHeader;
import com.tuling.tulingmall.open.entity.RequestMsgHeaderV2;
import com.tuling.tulingmall.open.service.BusiService;
...
@RestController
@RequestMapping(value = "/tlmallOpen")
@Api("请求转发接口")
public class TransferController {
	 ...
	 */
	@ApiOperation(value = "Restful接口", notes = "Restful调试接口")
	...
	public Object gsRestInterface(@RequestBody String requestMessage, HttpServletRequest request) {
		...
		// 启动mysql连接重连测试
		hisRequestService.connectCheck();
		// 统一鉴权预留。
		...
		if(StringUtils.isEmpty(reqTransId)|| StringUtils.isEmpty(sysId)|| StringUtils.isEmpty(serviceCode)){
			...
			String hisRsponse = hisRequestService.getHisResp(reqTransId, serviceCode);// 查询历史记录
	...
	@ApiOperation(value = "异步业务接口", notes = "增加接口鉴权相关机制。需配合提供的客户端程序进行使用。")
	...
	public Object gsDecrypedInterfaceAsync(HttpServletRequest request) {
		...
		if(StringUtils.isEmpty(transId)|| StringUtils.isEmpty(sysId)|| StringUtils.isEmpty(serviceCode)){
			...
			String hisRsponse = hisRequestService.getHisResp(transId, serviceCode);// 查询历史记录
			if (StringUtils.isNotEmpty(hisRsponse)) {
				// 有历史记录，直接推送历史记录。
